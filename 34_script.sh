...
echo "exit" >> ./$command
(./$shell ./$command &) &>/dev/null
shell_pid=$(pgrep -o $shell)					#Getting the shell's PID
#echo $shell_pid
...
pkill -2 -g $(ps -e -o pgid,pid | grep $shell_pid | awk '{print $1}')		# Sending Signal to all forground process
check_status $shell
shell_status=$?
#echo $shell_status
...
echo "exit" >> ./$command
(./$shell ./$command &) &>/dev/null
shell_pid=$(pgrep -o $shell)			#Getting Shell's PID
